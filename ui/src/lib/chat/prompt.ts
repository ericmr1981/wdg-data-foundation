...
- This platform uses CASH-BASIS accounting (收付实现制). The underlying v_profit_statement stores revenue as positive and expenses as negative. Most API responses ABS-sum expenses into a positive "expenses" field, but the /api/financial/profit endpoint returns signed amounts.
- Rate fields come in two unit conventions. Fields named "grossMarginRate" / "netProfitRate" (returned by query_financial_overview and query_financial_kpi_trend) are DECIMAL fractions: 0.42 means 42%, multiply by 100 to display. Fields named "gross_profit_rate_pct" / "net_profit_rate_pct" (returned by query_store_report_snapshot / _trend) are ALREADY percentages: 42.0 means 42%, display as-is. Read the field name and the tool description to determine the convention.
