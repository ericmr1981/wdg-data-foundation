...
export async function GET(request: NextRequest) {
  try {
    ...
    const storeSql = store ? `AND store_code = '${esc(store)}'` : '';
    ...
    const channelMetricsResult = await pool.query(
      `SELECT channel, SUM(net_amt) AS qimai_net_amt FROM (
        ...
        WHERE biz_date <= '${dateLit}'::DATE ${storeSql}
      ) sub GROUP BY channel ORDER BY channel`
    );

    // --- bank entry by channel ---
    const bankEntryResult = await pool.query(
      `SELECT r.lvl2_code AS channel, COALESCE(SUM(t.in_amt), 0) AS bank_entry_amt
      ...
      WHERE r.direction = 'in' AND r.lvl1_code = 'REV_BIZ'
        AND t.txn_time::date <= '${dateLit}'::DATE ${bankStoreSql}
    ...
    const monthlyTrendQimai = await pool.query(
      ...
      FROM bonjur_ods.income_detail ${trendStoreSql}
    ...
    const monthlyTrendBank = await pool.query(
      ...
      FROM bonjur_dm.v_bank_txn_classified_v2 t
      JOIN bonjur_cfg.bank_rule_map r ON t.counterparty_name ILIKE '%' || r.match_value || '%'
      WHERE r.direction = 'in' AND r.lvl1_code = 'REV_BIZ' ${bankStoreSql}
      GROUP BY 1 ORDER BY 1`
    );
    ...
    const monthlyTrend = [...trendMap.entries()]
      ...
      .map(([month, v]) => ({ month, qimai_net_amt: v.qimai.toFixed(2), bank_entry_amt: v.bank.toFixed(2) }));
      ...
      `SELECT month, channel, COUNT(*) AS order_count, SUM(net_amt) AS unentered_amt FROM (
        SELECT to_char(biz_date, 'YYYY-MM') AS month,
          CASE WHEN '微信支付' = ANY(payment_methods) THEN 'WECHAT'
            ...
            ELSE 'OTHER' END AS channel, net_amt
        FROM bonjur_ods.income_detail
        WHERE third_party_txn_no IS NULL AND biz_date <= '${dateLit}'::DATE ${storeSql}
      ) sub GROUP BY month, channel ORDER BY month DESC, channel`
    );
    ...
    for (const channel of allChannels) {
      ...
      totalQimai += qimaiAmt;
      totalBank += bankAmt;
      channelMetrics.push({ channel, qimai_net_amt: qimaiAmt.toFixed(2), bank_entry_amt: bankAmt.toFixed(2), entry_rate: qimaiAmt > 0 ? (bankAmt / qimaiAmt * 100).toFixed(2) : '0.00' });
    }
    channelMetrics.push({ channel: 'TOTAL', qimai_net_amt: totalQimai.toFixed(2), bank_entry_amt: totalBank.toFixed(2), entry_rate: totalQimai > 0 ? (totalBank / totalQimai * 100).toFixed(2) : '0.00' });

    const unmatchedOrders = (unmatchedOrdersResult.rows as any[]).map((r: any) => ({
      month: r.month, channel: r.channel, order_count: r.order_count, unentered_amt: parseFloat(r.unentered_amt).toFixed(2),
    }));
