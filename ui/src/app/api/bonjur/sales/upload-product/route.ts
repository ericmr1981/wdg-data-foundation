...
// POST /api/bonjur/sales/upload-product - 上传 Bonjur 企迈商品销售明细 CSV
export async function POST(request: Request) {
  ...
  // 5. Save file to inputs/bonjur/{store}/product_sales/{yyyyMM}/
  const now = new Date();
  const yyyyMM = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
  const uploadDir = path.join(process.cwd(), '..', 'inputs', 'bonjur', store, 'product_sales', yyyyMM);
  ...
  const scriptPath = path.join(projectRoot, 'scripts', 'import_bonjur_product_sales.py');
