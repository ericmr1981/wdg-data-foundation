...
// 委托到 brand_tamkoko_dm.v_cash_register_meal_period_overview / _detail
...
export async function GET(request: NextRequest) {
    ...
    try {
        const { rows } = await pool.query(
            `SELECT * FROM brand_tamkoko_dm.${view} ${where} ORDER BY store_code${detail ? ', biz_date, meal_period' : ', month, meal_period'}`,
            params
        );
