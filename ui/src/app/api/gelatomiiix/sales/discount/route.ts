    ...
    try {
        ...
        const bandQuery = `
            SELECT
                date_trunc('month', i.biz_date)::date AS month,
                CASE
                    WHEN i.discount_amt = 0 THEN '0%'
                    WHEN 100.0 * i.discount_amt / i.gross_amt < 15 THEN '0-15%'
                    WHEN 100.0 * i.discount_amt / i.gross_amt < 50 THEN '15-50%'
                    ELSE '50%+'
                END AS disc_band,
                COUNT(*) AS order_cnt,
                ROUND(SUM(i.gross_amt)::numeric, 0) AS total_gross
            FROM ${ODS}.income_detail i
            ${whereMonth}
            GROUP BY month, disc_band ORDER BY month, disc_band`;
