...
import type { IncomeMetricsData, CounterpartySummary } from '@/lib/queries/income';
import {
...
interface IncomeClientProps {
  ...
  initialMetrics: IncomeMetricsData;
  initialCounterparties: CounterpartySummary[];
}

// ── Helper: Period options ───────────────────────────────────────────────────

function buildPeriodOptions(span: string): string[] {
  const base = ['all'];
...
function BankEntryRateSection({ brand, span, period, store, channel, onChannelChange }: {
  ...
  }, [brand, span, period, store, channel]);
...
// ---- SettlementCycleSection (Tamkoko 支付宝+微信对账) ----
...
// ---- TaobaoReconSection (Tamkoko 淘宝闪购对账) ----
...
// ---- MeituanReconSection (Tamkoko 美团外卖对账) ----
...
// ---- MeituanTuangouSection (Tamkoko 美团团购券对账) ----
...
// ---- DouyinReconSection (Tamkoko 抖音团购券对账) ----
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">匹配窗口</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">窗口天数</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">���行金额</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">企迈笔数</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">企迈金额</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">差额</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">入账率</th>
...
export function IncomeClient({
  ...
  // Client state for interactive features
  ...
  // Metrics from initialData (RSC pre-fetched)
  const [metrics] = useState(initialMetrics);

  // Counterparties from initialData (RSC pre-fetched)
  const counterparties = initialCounterparties;

  // buildPeriodOptions moved to module level, keep it for use in render
  const periodOptions = buildPeriodOptions(span);

  // Fetch detail when counterparty selected (client-side fetch, user interaction)
  useEffect(() => {
    if (!selected) return;
  ...
  function navigate(
    ...
    params.set('brand', b);
    if (s !== 'month') params.set('span', s);
    if (p !== 'all') params.set('period', p);
    if (st !== 'all') params.set('store', st);
    const qs = params.toString();
    router.push(`/u/income${qs ? '?' + qs : ''}`);
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <select value={brand} onChange={e => navigate({ brand: e.target.value, store: 'all' })} className="border rounded px-2 py-1 text-sm bg-white">
            {brandOptions.map(b => <option key={b.code} value={b.code}>{b.name}</option>)}
          </select>
          <select value={span} onChange={e => navigate({ span: e.target.value as 'month' | 'quarter' | 'year' })} className="border rounded px-2 py-1 text-sm bg-white">
            <option value="month">按月</option>
            <option value="quarter">按季</option>
            <option value="year">按年</option>
          </select>
          <select value={period} onChange={e => navigate({ period: e.target.value })} className="border rounded px-2 py-1 text-sm bg-white">
            {periodOptions.map(p => <option key={p} value={p}>{p === 'all' ? '全部' : p}</option>)}
          </select>
          <select value={store} onChange={e => navigate({ store: e.target.value })} className="border rounded px-2 py-1 text-sm bg-white">
