-- Tamkoko | 收银明细 日级 KPI 视图
-- 用于 UI drill-down:点击 12 月趋势某月 → 加载该月日级折线
-- 与 v_cash_register_overview 同公式,GROUP BY (store_code, biz_date)

CREATE OR REPLACE VIEW brand_tamkoko_dm.v_cash_register_daily AS
WITH base AS (
    SELECT
        store_code,
        biz_date,
