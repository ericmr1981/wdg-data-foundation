-- Yufeng｜L2 snapshot：分类结果落表（避免在线 view 触发全量分类）
-- 依赖：yufeng_dm.fn_classify_bank_txn_v2, yufeng_ods.bank_txn
-- 目标：提供稳定、可索引、可增量刷新的分类快照表

CREATE SCHEMA IF NOT EXISTS yufeng_dm;

CREATE TABLE IF NOT EXISTS yufeng_dm.bank_txn_classified_snapshot (
...
CREATE INDEX IF NOT EXISTS idx_yufeng_classified_snapshot_month_src
  ON yufeng_dm.bank_txn_classified_snapshot(month, classified_source);

CREATE INDEX IF NOT EXISTS idx_yufeng_classified_snapshot_source_file
  ON yufeng_dm.bank_txn_classified_snapshot(source_file_id);

-- 增量刷新：按 source_file_id（上传文件）或全量
CREATE OR REPLACE FUNCTION yufeng_dm.refresh_bank_txn_classified_snapshot(p_source_file_id INT DEFAULT NULL)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
  INSERT INTO yufeng_dm.bank_txn_classified_snapshot(
    bank_txn_id,
    source_file_id,
    month,
    matched_rule_id,
  ...
  WHERE c.bank_txn_id = t.id
    AND c.classified_source NOT IN ('override', 'ignore')
    AND EXISTS (
      SELECT 1 FROM yufeng_ods.bank_txn n
      WHERE n.id != t.id
...
-- 初始化建议：首次上线后执行一次全量刷新
-- SELECT yufeng_dm.refresh_bank_txn_classified_snapshot(NULL);
