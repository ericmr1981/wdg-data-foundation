-- Gelatomiiix | 蜜可诗 销售月度 KPI 概览
-- 源: gelatomiiix_ods.income_detail (WHERE NOT is_refund)
-- 输出: 每 (store_code, month) 一行, 含核心 KPI 与环比

CREATE OR REPLACE VIEW brand_gelatomiiix_dm.v_sales_overview AS
WITH base AS (
    SELECT
        store_code,
...
SELECT
    ...
    -- 上一期
    LAG(gross_amt)    OVER w AS prev_gross_amt,
    LAG(revenue_amt)  OVER w AS prev_revenue_amt,
    LAG(net_amt)      OVER w AS prev_net_amt,
    LAG(order_cnt)    OVER w AS prev_order_cnt,
    -- 环比变化
