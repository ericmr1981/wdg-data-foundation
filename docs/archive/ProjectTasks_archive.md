...
- **ID**: WDG
- **变更记录**:
  - 2026-03-26：生产稳定性专项（L2 snapshot 主线启动）：修复 VPS 超时/雪崩（bonjur classified view 改为 LATERAL 单次调用）；Pipeline KPI 改为快速路径避免全量分类；Compose 补齐挂载（`sql/`/`ops/`）实现 dev/VPS 环境等价；L2 snapshot 方案进入实施（in_progress）。
  - 2026-03-26：本地 Metabase 看板优化：筛选器改为 Month(月粒度) + 门店(下拉) + 支出一级 + 收入一级；移除“一/二级”筛选；修复 Cards SQL（`lvl1_name/lvl2_name` 列），并更新利润口径为 `profit_amt = 营业收入 - 支出总金额(不含营建)`（DB 视图 `yufeng_dm.profit_monthly`）。
  - 2026-03-26：新增 `FieldTree.md`（Excalidraw）用于沉淀“数据处理 / BI 展示 / 人工分析”的分工边界与财务指标目标（现金流/支出健康/盈利）。
...
- **Round goal**：本地 Metabase 榆枫与山经营看板可用性修复 + 筛选器按产品要求重整 + 利润口径落库
- **Changes**：
  - `scripts/metabase_seed_dashboard.py`：修复 Cards SQL（`lvl1_name/lvl2_name`）、看板筛选器重整（月/门店/支出一级/收入一级）
  - `sql/yufeng_dm_models.sql` + DB：`yufeng_dm.profit_monthly` 增加 `expense_ex_build_amt` 并将 `profit_amt = bank_revenue_amt - expense_ex_build_amt`
- **Verification**：
  - Metabase：`http://localhost:3001/dashboard/40` 卡片全量可显示
  - API：`POST /api/card/44/query` 返回 202 且有数据
- **Result**：通过
- **Decision**：keep
- **Next**：门店筛选如需“真正下拉(非输入补全)”→ 改为 Metabase Field Filter（绑定 dim_store）
...
- **Round goal**：L2 snapshot 基建（分类落表/增量刷新）+ dev/VPS 环境等价
- **Changes**：
  - Repo：`docker-compose.dashboard.yml` 补齐挂载 `sql/`、`ops/`
  - Repo：新增 `sql/yufeng_classification_snapshot.sql`、`sql/bonjur_classification_snapshot.sql`
  - Repo：`scripts/import_yufeng_bank_txn.py` 导入成功后 best-effort 刷新 snapshot
  - Repo：优化 `sql/yufeng_fn_classify_v2.sql` / `sql/bonjur_fn_classify_v2.sql`（LATERAL 单次调用，避免雪崩）
- **Verification**：
  - 本地 DB：`select yufeng_dm.refresh_bank_txn_classified_snapshot(null);` 后 `snapshot_rows == yufeng_ods.bank_txn rows`（312==312）
- **Result**：通过
- **Decision**：keep
- **Next**：上线 VPS 后补齐“规则沉淀/重跑”自动刷新 snapshot（避免规则改了但页面不更新）+ 持续完善 smoke/回滚

- **Round goal**：VPS 线上切换到 snapshot 读取 + 把“规则沉淀/重跑”变成真正的增量重算
- **Changes**：
  - VPS：apply `*_classification_snapshot.sql`（表+增量刷新函数）并执行一次 `refresh_*_snapshot(NULL)` 回填
  - VPS：切换 `*_dm.v_bank_txn_classified` 为 join snapshot（保留 yufeng legacy 列 `lvl1/lvl2`）
  - Repo：`/api/rules/settle`、`/api/rules/settle-batch` 在写规则后 best-effort 刷新对应 `source_file_id` 的 snapshot
  - Repo：`/api/pipeline/rerun-match-by-file` 现在会对目标文件执行 `refresh_*_snapshot(file_id)`（真正“重跑”）
- **Verification**：
  - VPS：登录态下 `/api/rules?brand_code=bonjur`、`/api/match?brand_code=bonjur&month=2026-03` HTTP 200 且 ~10ms
- **Result**：通过（登录态）
- **Decision**：keep
- **Next**：将“刷新 snapshot”的动作接入更多写路径（规则新增/删除/导入）、并补一份 ops 脚本（按 brand+file 一键 refresh）

#### Doing（正在收口）
- [ ] L2 snapshot 主线（分类结果落表 + 增量刷新 + UI/Metabase 只读快表）
  - 目标：彻底消除 VPS 上 view/接口触发全量分类导致的超时与“改 A 出 B/C”
  - 范围：yufeng + bonjur
  - 验证：UI（/pipeline /match /rules /upload）秒开；Metabase 看板/下钻卡可用；guard 全绿
