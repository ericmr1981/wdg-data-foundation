...
export function encrypt(plaintext, userKey) {
    if (!userKey)
        throw new SecretCryptoError('Encryption key required');
...
export function decrypt(ciphertext, userKey) {
    if (!userKey)
        throw new SecretCryptoError('Encryption key required');
    const key = deriveKey(userKey);
    let buf;
...
export function _getKey(userKey) { return deriveKey(userKey); }
