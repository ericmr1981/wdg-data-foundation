...
try {
    ...
    log('warn', 'access log open failed', { err: err.message });
}
...
const proxyHttp = (clientReq, clientRes) => {
    ...
    upstream.on('response', (upRes) => {
        ...
        writeAccessLog(formatAccessLine(clientReq, upRes.statusCode ?? 502, clientRes.getHeader('content-length')));
    });
...
const formatAccessLine = (req, status, size) => {
...
const proxyWs = (clientReq, clientSocket, head) => {
    const upstream = http.request({
        host: AGENT_WS_HOST,
        port: AGENT_WS_PORT,
        method: 'GET',
