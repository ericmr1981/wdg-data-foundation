...
export class WebChannel {
            ...
            ws.on('message', (raw) => {
                ...
                if (!client.authed) {
                    ...
                    try {
                        ...
                        const msg = e.message;
                        console.error('[ws] auth failed:', msg, e.stack?.slice(0, 200));
                ...
                if (frame.type === 'user.message') {
                    ...
                    this.onUserMessage(client, frame.payload).catch((e) => {
                        const m = e.message ?? String(e);
                        console.error('[ws] onUserMessage ERROR:', m, e.stack?.slice(0, 300));
                        emitter.send({ type: 'error', payload: { code: 'invalid_request', http_status: 500, message: m } }).catch(() => { });
                    });
                    return;
                }
                if (frame.type === 'user.interrupt') {
                    this.onUserInterrupt(client, frame.payload).catch((e) => {
                        console.error('[ws] onUserInterrupt error:', e.message);
                    });
                    return;
                }
            });
    ...
    async onUserMessage(client, payload) {
    ...
    async onUserInterrupt(client, payload) {
     ...
     */
    async sendToConversation(conversationId, frame) {
        let sent = 0;
        for (const client of this.clients.values()) {
            if (client.conversationId === conversationId) {
                try {
                    ...
                    // 单 client 失败不影响其他
                    console.warn(`[web] sendToConversation client error: ${e.message}`);
                }
            }
        }
        if (sent === 0) {
