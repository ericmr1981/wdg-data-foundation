...
export function registerAdminSkillRoutes(app) {
    // 鉴权: 仅 admin
    ...
    // GET 单个
    app.get('/api/admin/skills/:name', async (req, reply) => {
    ...
    // PUT 更新
    app.put('/api/admin/skills/:name', async (req, reply) => {
        const file = findFileByName(req.params.name);
        if (!file)
            return reply.code(404).send({ error: 'skill not found' });
        ...
        try {
            ...
            return reply.code(400).send({ error: 'yaml frontmatter parse failed: ' + e.message });
        }
