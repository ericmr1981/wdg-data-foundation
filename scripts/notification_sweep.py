        ...
        cur.execute(
            ...
            INSERT INTO ops.notification
                (type, brand_code, severity, title, body,
                 action_url, action_label, related_id, dedup_key, swept_at)
            ...
            (type_, brand_code, severity, title, body,
             action_url, action_label, related_id, dedup_key),
        )
        row = cur.fetchone()
        conn.commit()
...
def sweep_unmatched_txn(conn, brands: list[str] | None = None) -> int:
    ...
    for brand in target_brands:
        ...
        new_count += upsert_notification(
            conn,
            type_='unmatched_txn',
            dedup_key=f'unmatched_txn:{brand}:{today_iso}',
            title=f'{brand} 有 {count} 条未配条目',
            body='点击查看未配条目并进行分析匹配',
            brand_code=brand,
            severity='warn',
